...
public class MagicCameraView extends MagicBaseView {
    ...
    public MagicCameraView(Context context, AttributeSet attrs) {
        ...
        //设置视频保存路劲和文件：
        String composeDir = MyFileUtil.DIR_VEDIO.toString() + File.separator;//路径(cache)
        String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.CHINESE).format(new Date());
//        File mediaFile = new File(composeDir + timeStamp + ".mp4");
        outputFile = new File(composeDir, timeStamp + ".mp4");
//        outputFile = new File(composeDir, MagicParams.videoName);
    ...
    public void savePicture(final SavePictureTask savePictureTask) {
        CameraEngine.takePicture(null, null, new Camera.PictureCallback() {
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                CameraEngine.stopPreview();//停止预览
                LogUtils.sysout("555555555555555 data = "+data.toString());
